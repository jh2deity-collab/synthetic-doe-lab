                    ...
                    {isGeneratingPdf && (
                        <div style={{ position: 'fixed', top: 0, left: 0, zIndex: 9000, backgroundColor: 'white' }}>
                            <ReportView
                                ...
                                insight={`이 분석은 두 그룹(A그룹과 B그룹)을 비교하여 얼마나 큰 차이가 있는지 측정한 결과입니다.

📊 핵심 결과:
• 그룹 A 평균: ${result.mean_a.toFixed(2)}
• 그룹 B 평균: ${result.mean_b.toFixed(2)}
• 평균 차이: ${Math.abs(result.mean_a - result.mean_b).toFixed(2)}
• Cohen's d: ${result.cohens_d.toFixed(2)} (${result.interpretation})

💡 쉬운 해석:
Cohen's d는 두 그룹의 차이를 표준화된 방식으로 측정한 값입니다. ${result.cohens_d.toFixed(2)}라는 값은 "${result.interpretation}"을 의미하며, 이는 ${Math.abs(result.cohens_d) < 0.2 ? '두 그룹이 거의 비슷하다' : Math.abs(result.cohens_d) < 0.5 ? '두 그룹에 작은 차이가 있다' : Math.abs(result.cohens_d) < 0.8 ? '두 그룹에 중간 정도의 차이가 있다' : '두 그룹에 매우 큰 차이가 있다'}는 것을 나타냅니다.

🎯 실용적 의미:
${Math.abs(result.cohens_d) >= 0.8 ? `이 정도 차이는 실제 현장에서도 명확하게 느낄 수 있을 만큼 큽니다. 예를 들어, 신약 효과 테스트라면 신약이 확실히 효과가 있다고 판단할 수 있습니다.` : Math.abs(result.cohens_d) >= 0.5 ? `이 정도 차이는 통계적으로 의미가 있지만, 실제 현장에서는 주의 깊게 관찰해야 알 수 있는 수준입니다.` : `이 정도 차이는 통계적으로 작은 편이며, 실제로는 거의 차이가 없다고 볼 수 있습니다.`}`}
                            />
                        </div>
                    )}
                </div>
